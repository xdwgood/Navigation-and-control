#include <ros/ros.h>
#include <ros_learn_action/FibonacciAction.h>  //包含action 消息头文件。
#include <actionlib/server/simple_action_server.h>  //包含SimpleActionServer头文件

//重定义SimpleActionServer类型为Server.
typedef actionlib::SimpleActionServer<ros_learn_action::FibonacciAction> Server;

//ActionServer的回调函数，类似服务的回调函数。
void executeCB(const ros_learn_action::FibonacciGoalConstPtr &goal, Server *as)
{
  ros_learn_action::FibonacciFeedback feedback; //定义反馈队列
  ros_learn_action::FibonacciResult result;     //定义结果队列
